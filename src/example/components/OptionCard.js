...
const OptionCard = ({ data, open }) => {
	...
	return (
		...
		<motion.div layout className="option-closed" style={{backgroundColor: color}} onClick={()=>setIsOpen(!isOpen)} whileHover={{ scale: 1.1 }}>
			<Icon size={35}/>
...
const Item =()=> {
	...
	return (
		<motion.li layout onClick={toggleOpen} initial={{ borderRadius: 10 }} className='item-card'>
			<motion.div layout onClick={toggleOpen} initial={{ borderRadius: 20 }} className='item-card'>
